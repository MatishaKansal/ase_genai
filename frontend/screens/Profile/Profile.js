    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.top}>
                ...
                <View style={styles.settings_container}>
                    <TouchableOpacity onPress={() => setDropdownOpen(!dropdownOpen)}>
                        <Ionicons name="settings-outline" size={34} color="#f59e0b" />
                    </TouchableOpacity>

                    {dropdownOpen && (
                        <View style={styles.dropdown}>
                            <Text style={styles.dropdown_item}>Edit Profile</Text>
                            <Text style={styles.dropdown_item}>Change Password</Text>
                            <View style={styles.hr} />
                            <TouchableOpacity onPress={handleLogout}>
                                <Text style={styles.dropdown_item}>Logout</Text>
                            </TouchableOpacity>
                        </View>
                    )}
                </View>
            ...
            <View style={styles.history}>
                <Text style={styles.historyHeading}>YOUR CHAT HISTORY ....</Text>
                ...
                {info.length === 0 ? (
                    ...
                    <FlatList
                        ...
                        style={{ flex: 1, maxHeight: height * 0.5 }}
                        renderItem={({ item, index }) => (
                            <View
                                style={[
                                    styles.historyCard,
                                    Platform.OS === "web" && hoverIndex === index && styles.historyCardHovered,
                                ]}
                                onMouseEnter={() => Platform.OS === "web" && setHoverIndex(index)}
                                onMouseLeave={() => Platform.OS === "web" && setHoverIndex(null)}
                            >
                                <LinearGradient
                                    colors={["rgba(31, 41, 55, 0.6)", "rgba(55, 65, 81, 0.4)"]}
                                    start={{ x: 0, y: 0 }}
                                    end={{ x: 1, y: 0 }}
